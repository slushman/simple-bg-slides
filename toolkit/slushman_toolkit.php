...
/**
 * A class of helpful WordPress functions
 *
...
/**
 * A multi-dimensional version of wp_parse_args(). Both params must be arrays.
 *
 ...
 * 
 * @param 	array 	$array1		Array of overriding options
 * @param 	array 	$array2		Array of default options
 *
 * @uses 	parse_args_recursive
 * 
 * @return 	array 	The parsed args array
 */
		public function parse_args_recursive( $args, $defaults = '' ) {
			...
			foreach( $args as $key => $value ) {
				...
				if ( is_array( $value ) && isset( $return[$key] ) ) {
					...
					$return[$key] = self::parse_args_recursive( $value, $return[$key] );
...
/* ==========================================================================
   ...
   @todo: take this out of here. Maybe a separate class? Integrate with
   existing make_fields class.
   ========================================================================== */
 ...
 */		
		function currencies( $params ) {
			...
			if ( !empty( $first ) ) {
				...
				$display = ( TRUE === $show ? $currencies[$first] . ' - ' . $first : $currencies[$first] );
			...
			foreach ( $currencies as $name => $symbol ) {
				...
				$display = ( TRUE === $show ? $symbol . ' - ' . $name : $symbol );
